...
async function send(text) {
	...
	// const dataToSend = JSON.stringify({"msg": text});
	// let dataReceived = "";
	// fetch(baseUrl, {
	// 	method: "post",
	// 	body: dataToSend
	...
	// 	.then(resp => {
	// 		if (resp.status === 200) {
	// 			newRecievedMessage(JSON.stringify(resp.json()));
	//
	// 		} else {
	...
	// 	})
	// 	.then(dataJson => {
	// 		newRecievedMessage(JSON.stringify(dataJson));
	// 	})
	// 	.catch(err => {
...
// 	})
// 	newRecievedMessage(JSON.stringify(response.json()));
...
function createNewMessage(message) {
	...
	$chatlogs.append(
		$('<div/>', {'class': 'chat friend'}).append(
			$('<div/>', {'class': 'user-photo'}).append($('<img src="Images/chatbot.png" />')),
